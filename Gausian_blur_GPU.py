...
def InitGL(Width, Height, texture_image):
    ...
    program = compileProgram(
        ...
        compileShader('''
            ...
            float offset[3] = float[](1.0, 1.3, 3.2);
            float weight[3] = float[](0.2, 0.3, 0.07);

            void main() {
                vec2 txy = pos.xy;
                vec3 tc = vec3(1.0, 0.0, 0.0);
                tc = texture2D(s_texture, txy).rgb * weight[0];
                for(int i=1; i<3; i++)
                {
                    tc += texture2D(s_texture, txy + vec2(0.0, offset[i])/texture_height).rgb * weight[i];
                    tc += texture2D(s_texture, txy - vec2(0.0, offset[i])/texture_height).rgb * weight[i]; 
                }
...
window = glutCreateWindow("Gausian_blur_GPU")
